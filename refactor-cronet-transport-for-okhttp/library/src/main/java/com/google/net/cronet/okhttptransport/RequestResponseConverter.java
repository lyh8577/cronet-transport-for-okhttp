...
final class RequestResponseConverter {
    ...
    private void applyRequestHeadersAndCookies(UrlRequest.Builder builder, Request request, CookieJar cookieJar) {
        ...
        for (int i = 0; i < headers.size(); i++) {
            ...
            if (!HttpUtil.COOKIE_HEADER_NAME.equalsIgnoreCase(name))
                builder.addHeader(name, headers.value(i));
        }

        final var cookieHeader = HttpUtil.cookieHeader(cookieJar, request.url(), headers);
        if (!cookieHeader.isEmpty())
            builder.addHeader(HttpUtil.COOKIE_HEADER_NAME, cookieHeader);
    }
